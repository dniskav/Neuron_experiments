...
import { generarDatos, clasificar, type MuestraColor } from "../../data/datosColor";
...
export function useColorTraining() {
  ...
  const entrenar = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      nueva1.weights = [...n1.weights]; nueva1.bias = n1.bias;
      const nueva2 = crearNeuronaN(CLAVE_N2);
      nueva2.weights = [...n2.weights]; nueva2.bias = n2.bias;
      ...
      for (let e = 0; e < n; e++) {
        ...
        if ([...nueva1.weights, nueva1.bias, ...nueva2.weights, nueva2.bias].some(isNaN)) {
          nueva1.weights = [0, 0, 0]; nueva1.bias = 0;
          nueva2.weights = [0, 0, 0]; nueva2.bias = 0;
          break;
        }
      }
      setN1(nueva1);
  ...
  const muestras = useMemo(() => MUESTRAS.map(m => {
