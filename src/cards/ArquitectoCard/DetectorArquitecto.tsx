...
import { useTutor } from './hooks/tutor/useTutor'
import { type OptimizerType, type LayerConfig } from './hooks/training/useNetworkConfig'
import { useNetworkConfig } from './hooks/training/useNetworkConfig'
import { useClassificationTraining } from './hooks/training/useClassificationTraining'
...
const SUCCESS_THRESHOLD: Record<number, number> = { 1: 0.95, 2: 0.9, 3: 0.85 }
...
export function DetectorArquitecto({ problemIds }: DetectorArquitectoProps = {}) {
  const visibleProblems = problemIds ? PROBLEMS.filter((p) => problemIds.includes(p.id)) : PROBLEMS
  ...
  // Configuración inicial por problema
  const defaultConfig = {
    hiddenLayers: [],
    optimizerType: 'adam' as OptimizerType,
    lr: selectedProblem.nivel >= 3 ? 0.01 : 0.05,
    epf: 20
  }
  const config = useNetworkConfig(
    defaultConfig.hiddenLayers,
    defaultConfig.optimizerType,
    defaultConfig.lr,
    defaultConfig.epf
  )
  const t = useClassificationTraining(selectedProblem, config)
  ...
  const networkLayers = useMemo(
    () => [
      { size: 2 },
      ...config.hiddenLayers.map((l: LayerConfig) => ({
        size: l.neurons,
  ...
  return (
    <Flex direction="column" gap={6} align="center" w="100%">
      ...
      {selectedProblem.id === 'snake' ? (
        <SnakeArquitecto />
      ) : selectedProblem.id === 'maze' ? (
        <MazeArquitecto />
      ) : (
        <Flex gap={8} align="flex-start" flexWrap="wrap" justify="center">
          ...
          <Flex direction="column" gap={4} maxW="380px" w="100%">
            ...
            <Box bg="white" border="1px solid" borderColor="gray.100" borderRadius="xl" p={4}>
              <Flex align="center" gap={2} mb={2}>
                ...
                <Box
                  ...
                  py="1px"
                  borderRadius="full"
                  bg={{ 1: '#10b981', 2: '#f59e0b', 3: '#ef4444' }[selectedProblem.nivel]}>
                  {selectedProblem.nivel === 1
                    ? 'FÁCIL'
                    : selectedProblem.nivel === 2
                      ? 'MEDIO'
                      : 'DIFÍCIL'}
                </Box>
              </Flex>
            ...
            <NetworkDiagram
              ...
              optimizer={`${OPT_LABEL[config.optimizerType]} lr=${config.lr}`}
            ...
            <Box bg="gray.50" border="1px dashed" borderColor="gray.200" borderRadius="xl" p={3}>
              ...
              <LayerBuilder
                hiddenLayers={config.hiddenLayers}
                onAdd={config.addLayer}
                onRemove={config.removeLayer}
                onUpdate={config.updateLayer}
              />
            </Box>
            ...
            <Flex gap={2} flexWrap="wrap" align="center">
              <Flex align="center" gap={1}>
                ...
                <NativeSelect.Root size="sm" w="auto">
                  <NativeSelect.Field
                    ...
                    onChange={(e) => config.setOptimizerType(e.target.value as OptimizerType)}
                    fontSize="12px">
              ...
              <Flex align="center" gap={1}>
                ...
                <NativeSelect.Root size="sm" w="auto">
                  <NativeSelect.Field
                    value={config.lr}
                    onChange={(e) => config.setLr(Number(e.target.value))}
                    fontSize="12px">
              ...
              <Flex align="center" gap={1}>
                ...
                <NativeSelect.Root size="sm" w="auto">
                  <NativeSelect.Field
                    value={config.epf}
                    onChange={(e) => config.setEpf(Number(e.target.value))}
                    fontSize="12px">
                    {[1, 5, 20, 50, 100].map((v) => (
                      <option key={v} value={v}>
                        {v} ép/f
            ...
            {/* Tutor contextual */}
            {!isSolvedNow && tutorMsg && <TutorPanel msg={tutorMsg} />}
            ...
            <DetailsBox summary="¿Por qué importa la arquitectura?">
              <BodyText>
                <strong>Lineal vs no-lineal:</strong> sin capas ocultas, la red solo puede trazar
                una línea recta. Muchos problemas (XOR, círculo, espiral) requieren fronteras curvas
                — imposibles sin neuronas ocultas.
              </BodyText>
              <BodyText>
                <strong>Truco del linear:</strong> añade capas ocultas con activación
                <em> linear</em> — por muchas que pongas, matemáticamente equivalen a una sola
                neurona. La no-linealidad (ReLU, Tanh…) es lo que da potencia real.
              </BodyText>
              <BodyText>
                <strong>La espiral</strong> es el reto definitivo: necesitas 2-3 capas, 8-16
                neuronas y la activación correcta. Con sigmoid suele colapsar; prueba ReLU o Tanh.
              </BodyText>
            </DetailsBox>
          </Flex>
        </Flex>
