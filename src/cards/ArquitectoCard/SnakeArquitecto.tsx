...
import { useSnakeRL } from "./useSnakeRL";
import { useTutorSnake } from "./useTutorSnake";
...
export function SnakeArquitecto() {
  ...
  const currentKey = s.hiddenLayers.map(l => `${l.neurons}${l.activation[0]}`).join("-") || "empty";
    ...
    <Flex direction="column" gap={5} align="center" w="100%">
      ...
      >
        <Flex align="center" gap={2} mb={2}>
          ...
          <Heading size="sm" color="gray.100">Snake — Agente DQN</Heading>
          <Box
        ...
        <Text fontSize="13px" color="gray.400" lineHeight={1.6}>
          ...
          Diseña la arquitectura: la red recibe <strong style={{ color: "#a7f3d0" }}>24 sensores</strong> (distancias
          a paredes y cuerpo en 8 direcciones, posición de la comida, dirección actual)
          y devuelve <strong style={{ color: "#a7f3d0" }}>3 Q-values</strong> (recto · derecha · izquierda).
      ...
      <Flex gap={8} align="flex-start" flexWrap="wrap" justify="center">
        ...
        <Flex direction="column" gap={3} align="center">
          ...
          {!s.demo ? (
            <SimpleGrid columns={4} gap={2} w="100%">
              ...
              <StatItem variant="compact" label="Explorar"  value={epsStr}
                highlight={s.stats.epsilon < 0.15}
              />
            </SimpleGrid>
          ) : (
            <SimpleGrid columns={3} gap={2} w="100%">
              <StatItem variant="compact" label="Demo actual" value={s.stats.demoScore}
                highlight={s.stats.demoScore > 0}
              />
              <StatItem variant="compact" label="Mejor demo"  value={s.stats.demoBest}
        ...
        <Flex direction="column" gap={4} maxW="360px" w="100%">
            ...
            <Flex align="center" gap={1}>
              ...
              <NativeSelect.Root size="sm" w="auto">
                <NativeSelect.Field
                  ...
                  onChange={e => s.setOptimizerType(e.target.value as "sgd" | "momentum" | "adam")}
                  fontSize="12px"
                  style={{ background: "transparent", color: "#cbd5e1" }}
                >
          ...
          <DetailsBox summary="¿Cómo aprende la serpiente? DQN explicado">
            ...
            <Text fontSize="12px" color="gray.600" lineHeight={1.7} mb={2}>
              <strong>Replay buffer:</strong> almacena 2000 experiencias pasadas.
              En cada paso, entrena con 32 experiencias <em>aleatorias</em> — no las más recientes.
              Esto rompe la correlación temporal y estabiliza el aprendizaje.
            </Text>
            <Text fontSize="12px" color="gray.600" lineHeight={1.7}>
              <strong>ε-greedy:</strong> al principio la serpiente explora aleatoriamente (ε≈1).
              A medida que aprende, ε decae hacia 0.05 y empieza a explotar lo aprendido.
            </Text>
          </DetailsBox>
          ...
          <DetailsBox summary="¿Qué ven los 24 sensores?">
            <Text fontSize="12px" color="gray.600" lineHeight={1.7} mb={2}>
              <strong>8 distancias a pared</strong> (N NE E SE S SW W NW): codificadas como 1/distancia.
              Cerca de la pared → valor alto; lejos → valor bajo.
            </Text>
            <Text fontSize="12px" color="gray.600" lineHeight={1.7} mb={2}>
