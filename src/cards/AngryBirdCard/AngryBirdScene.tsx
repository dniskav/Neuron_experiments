...
function TargetMesh({ escenaRef, hoverRef, anguloPredRef }: {
  ...
  return (
    <>
      ...
      <Text ref={warnRef as any} position={[0, CH / 2 - 60, 0.3]} fontSize={12} color="#ef4444" anchorX="center" anchorY="middle">
...
function TrajectoryLines({ escenaRef, anguloPredRef }: {
  escenaRef: RefObject<Escena>;
  anguloPredRef: RefObject<number | null>;
}) {
  const { correctLine, predLine, predMat, peakMesh, peakMat } = useMemo(() => {
    ...
    const correctMat = new THREE.LineDashedMaterial({ color: 0xffffff, opacity: 0.65, transparent: true, dashSize: 6, gapSize: 5 });
...
function Legend() {
  const { dashLine, solidLine } = useMemo(() => {
    ...
    const dashLine  = mkLine(new THREE.LineDashedMaterial({ color: 0xffffff, opacity: 0.65, transparent: true, dashSize: 5, gapSize: 4 }), 14);
  ...
  return (
    <>
      ...
      <Text position={[lx, ly1, 0.3]} fontSize={9} color="#ffffffd9" anchorX="left" anchorY="middle">trayectoria correcta</Text>
