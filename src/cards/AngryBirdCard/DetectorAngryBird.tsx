...
export function DetectorAngryBird() {
  ...
  const drag = useAngryBirdDrag(
    (esc) => training.getPred(esc),
    anguloPredRef,
    setAnguloPred,
  );

  const training = useAngryBirdTraining(
    anguloPredRef,
    drag.escenaRef,
    setAnguloPred,
    (v) => { drag.resetEscena(); setEscenaState(v); },
  );
  ...
  return (
    <CardRoot maxW="420px" w="100%">
      <Heading size="md" color="gray.800">Red de 3 capas · Tiro parabólico</Heading>
      <Text fontSize="sm" color="gray.600" lineHeight={1.5}>
        ...
        Arrastra el <strong>blanco ↔</strong> o el tope del <strong>obstáculo ↕</strong>{" "}
        para ver si la red generaliza a nuevos escenarios sin reentrenarse.
      </Text>
      ...
      >
        ...
        >
          <AngryBirdScene escenaRef={drag.escenaRef} anguloPredRef={anguloPredRef} hoverRef={drag.hoverRef} />
        </Canvas>
      </Box>
      ...
      <HStack gap={3}>
        <Box
          ...
          borderColor="gray.200"
        >
          <Text fontSize="10px" color="gray.500" textTransform="uppercase" letterSpacing={1}>Ángulo correcto</Text>
        ...
        <Box
          ...
          borderColor="gray.200"
        >
          <Text fontSize="10px" color="gray.500" textTransform="uppercase" letterSpacing={1}>Predicción de la red</Text>
          <Text fontSize="2xl" fontWeight={700} color={preciso ? "green.500" : "red.500"}>{predDeg}°</Text>
        </Box>
      </HStack>
      ...
      <DetailsBox summary="¿Por qué 3 capas y no 2?">
        <BodyText><strong>Capa 1 (16 neuronas):</strong> aprende la relación básica entre distancia al blanco y ángulo — una relación de arcoseno, altamente no lineal.</BodyText>
        <BodyText><strong>Capa 2 (8 neuronas):</strong> aprende a corregir ese ángulo según el obstáculo. ¿Cuánto más alto hay que tirar para superar ese muro en esa posición?</BodyText>
        <BodyText><strong>Al arrastrar</strong> el blanco u obstáculo puedes ver si la red <em>generalizó</em> (aprendió la física) o solo <em>memorizó</em> los ejemplos de entrenamiento. Una red bien entrenada debería predecir bien también en posiciones que nunca vio.</BodyText>
      </DetailsBox>
    </CardRoot>
  );
}
