...
import type { CañonSceneProps } from "./types";
...
export function wx(x: number) { return 14 + (x / MAX_X) * (CW - 28); }
export function wy(y: number) { return PY_SUELO - (y / MAX_Y) * (PY_SUELO - 8); }
export function px2wx(px: number) { return Math.max(0, Math.min(MAX_X, (px - 14) / (CW - 28) * MAX_X)); }
export function py2wy(py: number) { return Math.max(0, Math.min(MAX_Y, (PY_SUELO - py) / (PY_SUELO - 8) * MAX_Y)); }

// ─── Validación física ────────────────────────────────────────────────────────
export function tiroCorrecto(esc: Escena, angulo: number, v0: number): boolean {
  const xLand  = xAterrizaje(angulo, v0);
  const supera = alturaEn(esc.xObstaculo, angulo, v0) >= esc.hObstaculo;
  return Math.abs(xLand - esc.xBlanco) < 1.0 && supera;
}
...
function Background() {
  const meshes = useMemo(() => {
    ...
    const skyMat = new THREE.ShaderMaterial({
      uniforms: {
        uTop: { value: new THREE.Color("#312e81") },
        uBot: { value: new THREE.Color("#6366f1") },
      },
...
function ObstacleMesh({ escenaRef, hoverRef }: { escenaRef: RefObject<Escena>; hoverRef: RefObject<"target" | "obstacle" | null> }) {
  ...
  return (
    <>
      ...
      <Text ref={arrowRef as any} fontSize={9} color="#374151" anchorX="center" anchorY="middle">↕</Text>
    </>
  );
}
...
function TargetMesh({ escenaRef, hoverRef, predRef }: {
  escenaRef: RefObject<Escena>;
  hoverRef: RefObject<"target" | "obstacle" | null>;
  predRef: RefObject<{ angulo: number; v0: number } | null>;
}) {
  const haloFillMat = useMemo(() => new THREE.MeshBasicMaterial({ color: "#10b981", opacity: 0.35, transparent: true }), []);
  const haloRingMat = useMemo(() => new THREE.MeshBasicMaterial({ color: "#10b981", side: THREE.DoubleSide }), []);
  const haloFill    = useMemo(() => new THREE.Mesh(new THREE.CircleGeometry(26, 32), haloFillMat), [haloFillMat]);
  const haloRing    = useMemo(() => new THREE.Mesh(new THREE.RingGeometry(20.5, 23.5, 32), haloRingMat), [haloRingMat]);

  const ringMats = useMemo(() => RING_NORMAL.map(c => new THREE.MeshBasicMaterial({ color: c })), []);
  const rings    = useMemo(() => ringMats.map(mat => new THREE.Mesh(new THREE.CircleGeometry(1, 32), mat)), [ringMats]);
  ...
  useFrame(() => {
    ...
    const golpea = pred !== null && tiroCorrecto(escenaRef.current, pred.angulo, pred.v0);
    const optimo = encontrarOptimo(escenaRef.current);
    ...
    if (warnRef.current) warnRef.current.visible = optimo === null;
  });
...
function LauncherMesh() {
  const meshes = useMemo(() => {
    ...
    const dome = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: "#818cf8" }),
    );
    dome.position.set(lx, GROUND_Y, 0.2);

    const pole = new THREE.Mesh(
      new THREE.PlaneGeometry(6, 14),
      new THREE.MeshBasicMaterial({ color: "#312e81" }),
    );
...
function TrajectoryLines({ escenaRef, predRef }: {
  escenaRef: RefObject<Escena>;
  predRef: RefObject<{ angulo: number; v0: number } | null>;
}) {
  const { optLine, predLine, predMat, peakMesh, peakMat } = useMemo(() => {
    ...
    const optMat  = new THREE.LineDashedMaterial({ color: 0xffffff, opacity: 0.6, transparent: true, dashSize: 6, gapSize: 5 });
    const predMat = new THREE.LineBasicMaterial({ color: "#ef4444" });
    const peakMat = new THREE.MeshBasicMaterial({ color: "#ef4444" });

    return {
      optLine:  mkLine(optMat),
      predLine: mkLine(predMat),
      predMat,
      peakMesh: new THREE.Mesh(new THREE.CircleGeometry(7, 16), peakMat),
      peakMat,
    };
  }, []);
  ...
  useFrame(() => {
    ...
    // Trayectoria óptima
    if (optimo) {
      const pts = simularTrayectoria(optimo.angulo, optimo.v0);
      setLinePoints(optLine, pts);
      optLine.computeLineDistances();
      optLine.visible = true;
    } else {
      optLine.visible = false;
    }

    // Trayectoria predicha
    if (pred) {
      const pts = simularTrayectoria(pred.angulo, pred.v0);
      setLinePoints(predLine, pts);
      const superaObs = alturaEn(escena.xObstaculo, pred.angulo, pred.v0) >= escena.hObstaculo;
      const golpea    = tiroCorrecto(escena, pred.angulo, pred.v0);
      const color     = !superaObs ? "#ef4444" : golpea ? "#10b981" : "#fbbf24";
...
function Legend() {
  const { dashLine, solidLine } = useMemo(() => {
    const mkLine = (mat: THREE.Material, y: number) => {
      const geo = new THREE.BufferGeometry();
      geo.setAttribute("position", new THREE.BufferAttribute(new Float32Array([...ct(8, y), ...ct(30, y)]), 3));
      return new THREE.Line(geo, mat);
    };
    const dashLine  = mkLine(new THREE.LineDashedMaterial({ color: 0xffffff, opacity: 0.6, transparent: true, dashSize: 5, gapSize: 4 }), 14);
...
// ─── Escena principal ─────────────────────────────────────────────────────────
export function CañonScene({ escenaRef, predRef, hoverRef }: CañonSceneProps) {
  return (
    <>
      <Background />
      <LauncherMesh />
      <ObstacleMesh  escenaRef={escenaRef} hoverRef={hoverRef} />
      <TargetMesh    escenaRef={escenaRef} hoverRef={hoverRef} predRef={predRef} />
      <TrajectoryLines escenaRef={escenaRef} predRef={predRef} />
      <Legend />
    </>
  );
}
