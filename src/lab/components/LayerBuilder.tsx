...
import type { ActivationType } from './NetworkDiagram'

// Tipo genérico de capa oculta — compatible con LayerConfig de ArquitectoCard
// y HiddenLayer de RobotCard/SudokuCard.
...
interface Props {
  ...
  onUpdate:      (i: number, patch: Partial<HiddenLayerConfig>) => void
  maxLayers?:    number
  maxNeurons?:   number
  inputLabel?:   string
  outputLabel?:  string
...
function HiddenLayerCard({
  layer, index, onRemove, onUpdate, maxNeurons = 32,
}: {
  layer:      HiddenLayerConfig
  index:      number
  onRemove:   () => void
  onUpdate:   (patch: Partial<HiddenLayerConfig>) => void
  maxNeurons?: number
}) {
  return (
    <Flex
    ...
    >
      ...
      <Flex align="center" gap="2px">
        <Button size="xs" variant="ghost" px={1} h="18px" minW="18px"
        ...
        <Button size="xs" variant="ghost" px={1} h="18px" minW="18px"
          onClick={() => onUpdate({ neurons: Math.min(maxNeurons, layer.neurons + 1) })}>
          +
        </Button>
      </Flex>
      ...
      <Flex align="center" gap={1}>
        ...
        <Button size="xs" variant="ghost" colorPalette="red"
          px="2px" h="14px" fontSize="10px" onClick={onRemove}>
          ×
        </Button>
      </Flex>
    </Flex>
