...
function dotLayout(size: number): DotLayout {
  ...
  const realN    = hasMore ? shown - 1 : shown;
  const step     = DOTS_H / (shown + 1);
          ...
          return (
            <g key={i}>
              ...
              {overflow > 0 && overflowY !== null && (
                <text x={cx} y={overflowY + 4} textAnchor="middle"
                  fontSize={9} fill="#94a3b8" fontWeight="600">
                  +{overflow}
                </text>
              )}

              <text x={cx} y={DOTS_H + 14} textAnchor="middle"
                fontSize={12} fontWeight="700" fill="#1e293b">
                {layer.size}
              </text>

              <text x={cx} y={DOTS_H + 27} textAnchor="middle"
